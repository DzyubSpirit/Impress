...
impress.loadApplication = function(appName, dir, callback) {
  ...
  application.dir = dir;
  api.registry.mixinPluginsFromTo(impress, application);
...
impress.server.start = function() {
  api.registry.loadPluginsTo(impress);
...
impress.server.startup = function() {
  ...
  if (typeof(global.gc) === 'function' && impress.config.scale.gcInterval > 0) {
    api.timers.setInterval(global.gc, impress.config.scale.gcInterval);
  }
  isFirstStart = false;
...
impress.server.restart = function() {
  if (process.isMaster) console.log('Restarting...'.green);
  if (impress.config) impress.stop(function() {
    if (process.isMaster) console.log('  Reloading server configuration');
