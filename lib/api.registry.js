...
api.registry.modules = {
  ...
  index: { type: 'plugin' },
  files: { type: 'plugin' },
  templating: { type: 'plugin' },
  preprocess: { type: 'plugin' },
  security: { type: 'plugin' },
  state: { type: 'plugin' },
  cloud: { type: 'plugin' },
  sse: { type: 'plugin' },
  websocket: { type: 'plugin' },
  rpc: { type: 'plugin' },
  health: { type: 'plugin' },
  waf: { type: 'plugin' },
};
...
var loadFuncs = {
  'impress': function(moduleName) {
    api[moduleName] = {};
    require('./api.' + moduleName);
  },
  'node': function(moduleName) {
    api[moduleName] = api.registry.require(moduleName);
  },
  'npm': function(moduleName) {
...
api.registry.load = function() {
  ...
  var moduleName, moduleData, moduleType;
  for (moduleName in api.registry.modules) {
    moduleData = api.registry.modules[moduleName];
    moduleType = moduleData.type;
    if (moduleType in loadFuncs) {
      loadFuncs[moduleType](moduleName);
    }
  }

};

// Load core plugins to impress object
//
api.registry.loadPluginsTo = function(impress) {
  usePlugins(function(pluginName) {
...
// Mixin plugins that have impress object to application
//
api.registry.mixinPluginsFromTo = function(impress, application) {
  usePlugins(function(pluginName) {
    var plugin = impress[pluginName];
    application[pluginName] = {};
    if (plugin.mixin) plugin.mixin(application);
  });
};

// Apply usePluginFunc to plugins in api.registry
//
function usePlugins(usePluginFunc) {
  var plugins = api.registry.indexByType['plugin'];
  if (plugins !== undefined) {
    plugins.forEach(usePluginFunc);
  }
}
